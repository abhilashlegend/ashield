	...
	<tbody>
		<?php foreach( $value as $i => $row ): 
			...
			<tr class="acf-row<?php if( $i === 'acfcloneindex' ){ echo ' acf-clone'; } ?>" data-id="<?php echo $id; ?>">
				...
				<?php if( $show_remove ): ?>
					<td class="acf-row-handle remove" style="width:10%;">
						<a role="button" aria-label="Add row to <?php echo $field["label"]; ?>" class="button button-primary -plus small acf-js-tooltip asx-add-row-btn" href="#" data-event="add-row">Add Row</a>
						<a role="button" aria-label="Remove row from <?php echo $field["label"]; ?>" class="button button-primary -minus small acf-js-tooltip" href="#" data-event="remove-row">Remove Row</a>
						<?php /*
