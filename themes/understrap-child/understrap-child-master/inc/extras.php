		...
		<nav class="container navigation post-navigation">
			...
			<div class="row nav-links justify-content-between">
				...
				if ( get_previous_post_link() ) {
					...
					previous_post_link( '<span class="nav-previous">%link</span>', _x( '<i class="fa fa-angle-left"></i>&nbsp;' . wp_trim_words($title, 7), 'Previous post link', 'understrap' ) );
				}
				if ( get_next_post_link() ) {
					$next = get_next_post()->ID;
					$title = get_the_title( $next );
				
					next_post_link( '<span class="nav-next">%link</span>', _x( wp_trim_words($title, 7) . '&nbsp;<i class="fa fa-angle-right"></i>', 'Next post link', 'understrap' ) );
				}
				?>
			</div><!-- .nav-links -->
		</nav><!-- .navigation -->
